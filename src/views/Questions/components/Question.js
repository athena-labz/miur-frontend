...
const Question = ({
  question,
  options,
  powerUps, // Dictionary {powerup: {name: "PowerUp", used: boolean, onUse: () => void}}
  powerUpsDisabled,
  onAnswer,
  extraDisplay,
}) => {
  ...
  return (
    <div className="w-full">
      <div className="w-full flex items-center justify-center flex-col">
        ...
        <h1 className="text-center text-3xl text-white w-full">
          {question}
        </h1>
        ...
        <div className=" w-full mt-20 grid grid-cols-2 gap-x-20 gap-y-10">
          {Object.entries(options).map(([idx, option]) => (
            <div
              key={`option-${idx}`}
